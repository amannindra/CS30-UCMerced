...
struct Edge {
    ...
    Edge(Vertex *from, Vertex *to, int time, int cost) {
        this->from = from;
        this->to = to;
        this->time = time;
        this->cost = cost;
...
struct Waypoint {
    ...
    void expand() {
        for (int i = 0; i < vertex->edgeList.size(); i++) {
            ...
            temp->totalCost = this->totalCost + vertex->edgeList[i]->cost;

            // For BFS, partialCost doesn't matter much, but let's keep it clean
            temp->weight = 1;
            temp->partialCost = partialCost + 1;
    ...
    void expandTime() {
        for (int i = 0; i < vertex->edgeList.size(); i++) {
            Waypoint *temp = new Waypoint(vertex->edgeList[i]->to);
            temp->parent = this;
            temp->weight = vertex->edgeList[i]->time;
            temp->totalTime = this->totalTime + vertex->edgeList[i]->time;
            temp->totalCost = this->totalCost + vertex->edgeList[i]->cost;
    ...
    void expandCost() {
        for (int i = 0; i < vertex->edgeList.size(); i++) {
            ...
            temp->weight = vertex->edgeList[i]->cost;
...
struct Graph {
    ...
    void addEdge(Vertex *start, Vertex *end, int timed, int cost) {
        start->edgeList.append(new Edge(start, end, timed, cost));
        end->edgeList.append(new Edge(end, start, timed, cost));
    }
    ...
    void addDirectedEdge(Vertex *start, Vertex *end, int timed, int cost) {
        start->edgeList.append(new Edge(start, end, timed, cost));
    }

    Waypoint *bfs(Vertex *start, Vertex *destination) {
        std::cout << "Running Breadth-First Search for " << std::endl;
        ...
        while (!frontier.isEmpty()) {
            ...
            if (result->vertex == destination) {
                std::cout << "bfs result: " << result << std::endl;
                return result;
            }
    ...
    Waypoint *ucs(Vertex *start, Vertex *destination, int selectOption) {
        std::cout << "Running Uniform Cost Search with " << selectOption
                  << std::endl;
        ...
        while (frontier.size() != 0) {
            ...
            if (selectOption == 1) {
                std::cout << "Using expandTime for UCS" << std::endl;

                result->expandTime();
            }
            if (selectOption == 0) {
                std::cout << "Using expandCost for UCS" << std::endl;
                result->expandCost();
            }
            // result->expand();
